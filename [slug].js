...
// Vercel rewrite: /blog/:slug → /api/blog/:slug
// 검색엔진(구글/네이버)이 직접 이 URL을 인덱싱할 수 있게 메타태그 포함.
...
// 마크다운/평문을 HTML로 단순 변환 (안전하게)
const renderContent = (raw) => {
  if (!raw) return '';
  // 이미 HTML로 보이면 그대로 사용 (Notion sync는 보통 마크다운 또는 HTML 저장)
...
// 인라인 마크다운: **bold**, *italic*, [link](url), ![alt](src)
...
const renderPage = (post) => {
  ...
  const ldJson = {
    ...
    publisher: {
      '@type': 'Organization',
      name: '아트라이앵글',
      logo: { '@type': 'ImageObject', url: 'https://kwhrwbnfwoybdmdzmmko.supabase.co/storage/v1/object/public/photos/artryangle-logo.png' }
    },
...
<style>
  ...
  a { color: #8b6f47; padding: 12px 24px; border: 1px solid #1a1a1a; text-decoration: none; color: #1a1a1a; }
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // 2차 fallback: id(UUID) 또는 notion_id로 검색
    if (!post) {
